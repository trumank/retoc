...
#[rustfmt::skip]
const CRC_TABLE_DEPRECATED: [u32; 256] = [
...
fn generate_hash_deprecated(string: &str) -> u32 {
    let mut crc = 0u32;
    for c in string.to_ascii_uppercase().encode_utf16() {
        crc = ((crc >> 8) & 0xffffff) ^ CRC_TABLE_DEPRECATED[((crc ^ (c as u32)) & 0xff) as usize];
        if c > 0xff {
            crc = ((crc >> 8) & 0xffffff) ^ CRC_TABLE_DEPRECATED[((crc ^ (c as u32 >> 8)) & 0xff) as usize];
        }
    }
    crc
}

fn generate_crc32(string: &str) -> u32 {
    let mut crc = !0u32;
    for ch in string.encode_utf16() {
        let mut ch = ch as u32;
        crc = (crc >> 8) ^ CRC_TABLE_SB8[((crc ^ ch) & 0xff) as usize];
        ch >>= 8;
        crc = (crc >> 8) ^ CRC_TABLE_SB8[((crc ^ ch) & 0xff) as usize];
        ch >>= 8;
        crc = (crc >> 8) ^ CRC_TABLE_SB8[((crc ^ ch) & 0xff) as usize];
...
pub fn generate_name_hash(string: &str) -> (u16, u16) {
    let non_case_preserving = generate_hash_deprecated(string) as u16;
    let case_preserving = generate_crc32(string) as u16;
    (non_case_preserving, case_preserving)
}
