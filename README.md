...
### Overides & AES Keys
Overrides may be required to extract content from certain games. When repacking content to generate mods, the same override must be applied.

Likewise, if and when an AES key is required to decrypt a game's assets, it must be used to encrypt the mod.
